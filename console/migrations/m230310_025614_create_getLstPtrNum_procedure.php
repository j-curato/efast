...
/**
 * Class m230310_025614_create_getLstPtrNum_procedure
 */
class m230310_025614_create_getLstPtrNum_procedure extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        $sql = <<<SQL
            ...
            FROM ptr t, (SELECT @row := 0) r
            WHERE
            t.fk_office_id =office_id
            ),
            used_num as (
                SELECT CAST(SUBSTRING_INDEX(ptr_number,'-',-1) AS UNSIGNED) as p_number
                from ptr 
                WHERE  
            ptr.fk_office_id =office_id
            ),
            vacant_numbers as (
            SELECT 
            seq.`row` 
    ...
    {
        echo "m230310_025614_create_getLstPtrNum_procedure cannot be reverted.\n";
