function displayResultData(data) {
  ...
  $.each(data, function (index, val) {
    ...
    let total_debit_credit =
      val.total_debit_credit != null ? parseFloat(val.total_debit_credit) : 0;
    let balance = val.balance != null ? parseFloat(val.balance) : 0;
    let total = balance;
...
function query(csrfToken, reporting_period, book_type) {
  $.ajax({
    type: "POST",
    url: window.location.pathname + "?r=conso-sub-trial-balance/generate",
    data: {
