...
$this->title = "Annex 3: Report of Advances to Officers and Employees
";
...
<div class="jev-preparation-index" style="background-color: white;">
    <form id='filter'>
        <div class="row">
            ...
            <div class="col-sm-2">
                <label for="reporting_period">To Reporting Period</label>
                <?php
...
<script>
    ...
    $('#generate').click((e) => {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                console.log(res)
                // target_date = resreporting_period
    ...
    function display(data) {
        ...
        $.each(data, function(key, val) {
            ...
            const liquidated_amount = parseFloat(val.liquidated_amount)
            const particular = val.particular
            const payee = val.payee
            const total_disbursed = parseFloat(val.total_disbursed)
            let date1 = new Date($('#reporting_period').val());
            let date2 = new Date(issuance_date);
            let Difference_In_Time = date1.getTime() - date2.getTime();
            let Difference_In_Days = Difference_In_Time / (1000 * 3600 * 24);
            let less_30 = ''
            let between_31_to_60 = ''
            let between_61_to_365 = ''
            let over_year = ''
            if (Difference_In_Days <= 30 && balance != 0) {
                less_30 = thousands_separators(balance)
            } else if (Difference_In_Days >= 31 && Difference_In_Days <= 61 && balance != 0) {
                between_31_to_60 = thousands_separators(balance)
            } else if (Difference_In_Days >= 61 && Difference_In_Days <= 365 && balance != 0) {
                between_61_to_365 = thousands_separators(balance)
            } else if (Difference_In_Days > 365 && balance != 0) {
                var y = 0
                var m = 0
            ...
            let row = `<tr class='data_row'>
                    ...
                    <td  >` + particular + `</td>
                    <td  >` + check_number + `</td>
                    <td class='amount' >` + thousands_separators(total_disbursed) + `</td>
                    <td class='amount' >` + thousands_separators(liquidated_amount) + `</td>
