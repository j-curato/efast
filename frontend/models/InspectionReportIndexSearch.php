...
class InspectionReportIndexSearch extends InspectionReportIndex
{
    ...
    public function search($params)
    {
        ...
        if (!yii::$app->user->can('ro_inspection_admin')) {
            ...
            $query->andWhere('inspection_report_index.office_name = :office', ['office' => $user_data->employee->office->office_name]);
            if (!Yii::$app->user->can('ro_inspection_admin') && !Yii::$app->user->can('po_inspection_admin')) {
                $query->andWhere('inspection_report_index.division = :division', ['division' => $user_data->employee->empDivision->division ?? '']);
            }
        }
